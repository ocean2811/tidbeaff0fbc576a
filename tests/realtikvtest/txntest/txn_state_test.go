...
func TestRunning(t *testing.T) {
	...
	tk.MustExec("insert into t(a) values (1);")
	tk.MustExec("begin pessimistic;")
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/session/mockStmtSlow", "return(200)"))
	ch := make(chan struct{})
	go func() {
	...
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/session/mockStmtSlow"))
	<-ch
}
...
func TestCommitting(t *testing.T) {
	...
	go func() {
		...
		require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/session/mockSlowCommit", "pause"))
		tk2.MustExec("commit;")
		ch <- struct{}{}
	}()
	time.Sleep(100 * time.Millisecond)
	require.Equal(t, txninfo.TxnCommitting, tk2.Session().TxnInfo().State)
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/session/mockSlowCommit"))
	tk1.MustExec("commit;")
	<-ch
}
...
func TestRollbackTxnState(t *testing.T) {
	...
	go func() {
		...
		require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/session/mockSlowRollback", "pause"))
		tk.MustExec("rollback;")
		ch <- struct{}{}
	}()
	time.Sleep(100 * time.Millisecond)
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/session/mockSlowRollback"))
