...
import (
	...
	"github.com/ocean2811/tidbeaff0fbc576a/br/pkg/mock/mocklocal"
...
func (s *mockGCSSuite) TestGeneratedColumnsAndTSVFile() {
	// For issue https://github.com/ocean2811/tidbeaff0fbc576a/issues/39885
...
func (s *mockGCSSuite) TestImportMode() {
	...
	s.enableFailpoint("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/dispatcher/WaitCleanUpFinished", "return()")
	...
	switcher.EXPECT().ToImportMode(gomock.Any(), gomock.Any()).DoAndReturn(toImportModeFn).Times(1)
	switcher.EXPECT().ToNormalMode(gomock.Any(), gomock.Any()).DoAndReturn(toNormalModeFn).Times(1)
	s.enableFailpoint("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/importinto/clearLastSwitchTime", "return(true)")
	s.enableFailpoint("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/importinto/waitBeforePostProcess", "return(true)")
	...
	s.tk.MustExec("truncate table load_data.import_mode;")
	s.Greater(intoNormalTime, intoImportTime)
	s.NoError(failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/importinto/clearLastSwitchTime"))
	s.NoError(failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/importinto/waitBeforePostProcess"))
	<-dispatcher.WaitCleanUpFinished
	...
	s.enableFailpoint("github.com/ocean2811/tidbeaff0fbc576a/pkg/ddl/util/IsRaftKv2", "return(true)")
	s.tk.MustExec("truncate table load_data.import_mode;")
	sql = fmt.Sprintf(`IMPORT INTO load_data.import_mode FROM 'gs://test-load/import_mode-*.tsv?endpoint=%s'`, gcsEndpoint)
	s.tk.MustQuery(sql)
	s.tk.MustQuery("SELECT * FROM load_data.import_mode;").Sort().Check(testkit.Rows("1 11 111"))
	...
	s.NoError(failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/ddl/util/IsRaftKv2"))
	...
	s.NoError(failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/dispatcher/WaitCleanUpFinished"))
}
...
func (s *mockGCSSuite) TestRegisterTask() {
	...
	s.NoError(failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/importinto/errorWhenSortChunk"))
