...
func TestChecksumTable(t *testing.T) {
	...
	_ = failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/importinto/errWhenChecksum", `return(true)`)
	defer func() {
		_ = failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/importinto/errWhenChecksum")
	}()
	remoteChecksum, err = importinto.TestChecksumTable(ctx, mgr, taskMeta, logutil.BgLogger())
	require.NoError(t, err)
	require.True(t, remoteChecksum.IsEqual(&localChecksum))
