...
func TestFrameworkDynamicHA(t *testing.T) {
	...
	RegisterTaskMeta(t, ctrl, &m, &testDynamicDispatcherExt{})
	distContext := testkit.NewDistExecutionContext(t, 3)
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/dispatcher/mockDynamicDispatchErr", "5*return()"))
	DispatchTaskAndCheckSuccess(ctx, "key1", t, &m)
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/dispatcher/mockDynamicDispatchErr"))

	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/dispatcher/mockDynamicDispatchErr1", "5*return()"))
	DispatchTaskAndCheckSuccess(ctx, "key2", t, &m)
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/dispatcher/mockDynamicDispatchErr1"))

	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/dispatcher/mockDynamicDispatchErr2", "5*return()"))
	DispatchTaskAndCheckSuccess(ctx, "key3", t, &m)
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/dispatcher/mockDynamicDispatchErr2"))
	distContext.Close()
}
