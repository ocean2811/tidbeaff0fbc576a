...
import (
	...
	"github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/mock"
	mockexecute "github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/mock/execute"
	"github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/proto"
	"github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler"
...
func DispatchAndCancelTask(ctx context.Context, taskKey string, t *testing.T, m *sync.Map) {
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/MockExecutorRunCancel", "1*return(1)"))
	defer func() {
		require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/MockExecutorRunCancel"))
	}()
	task := DispatchTask(ctx, taskKey, t)
...
func DispatchMultiTasksAndOneFail(ctx context.Context, t *testing.T, num int, m []sync.Map) []*proto.Task {
	...
	for i := 0; i < num; i++ {
		if i == 0 {
			require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/MockExecutorRunErr", "1*return(true)"))
			...
			require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/MockExecutorRunErr"))
		} else {
...
func TestFrameworkSubTaskInitEnvFailed(t *testing.T) {
	...
	RegisterTaskMeta(t, ctrl, &m, &testDispatcherExt{})
	distContext := testkit.NewDistExecutionContext(t, 1)
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/mockExecSubtaskInitEnvErr", "return()"))
	defer func() {
		require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/mockExecSubtaskInitEnvErr"))
	}()
	DispatchTaskAndCheckState(ctx, "key1", t, &m, proto.TaskStateReverted)
	distContext.Close()
}
...
func TestOwnerChange(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/dispatcher/mockOwnerChange", "1*return(true)"))
	DispatchTaskAndCheckSuccess(ctx, "😊", t, &m)
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/dispatcher/mockOwnerChange"))
	distContext.Close()
}
...
func TestFrameworkCancelThenSubmitSubTask(t *testing.T) {
	...
	RegisterTaskMeta(t, ctrl, &m, &testDispatcherExt{})
	distContext := testkit.NewDistExecutionContext(t, 3)
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/dispatcher/cancelBeforeUpdate", "return()"))
	DispatchTaskAndCheckState(ctx, "😊", t, &m, proto.TaskStateReverted)
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/dispatcher/cancelBeforeUpdate"))
	distContext.Close()
}
...
func TestSchedulerDownBasic(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/mockCleanScheduler", "return()"))
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/mockStopManager", "4*return(\":4000\")"))
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/mockTiDBDown", "return(\":4000\")"))
	DispatchTaskAndCheckSuccess(ctx, "😊", t, &m)
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/mockCleanScheduler"))
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/mockTiDBDown"))
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/mockStopManager"))
	distContext.Close()
}
...
func TestSchedulerDownManyNodes(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/mockCleanScheduler", "return()"))
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/mockStopManager", "30*return(\":4000\")"))
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/mockTiDBDown", "return(\":4000\")"))
	DispatchTaskAndCheckSuccess(ctx, "😊", t, &m)
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/mockCleanScheduler"))
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/mockTiDBDown"))
...
func TestGC(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/storage/subtaskHistoryKeepSeconds", "return(1)"))
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/dispatcher/historySubtaskTableGcInterval", "return(1)"))
	defer func() {
		require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/storage/subtaskHistoryKeepSeconds"))
		require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/dispatcher/historySubtaskTableGcInterval"))
	}()
...
func TestFrameworkSubtaskFinishedCancel(t *testing.T) {
	...
	RegisterTaskMeta(t, ctrl, &m, &testDispatcherExt{})
	distContext := testkit.NewDistExecutionContext(t, 3)
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/MockSubtaskFinishedCancel", "1*return(true)"))
	defer func() {
		require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/MockSubtaskFinishedCancel"))
	}()
	DispatchTaskAndCheckState(ctx, "key1", t, &m, proto.TaskStateReverted)
	distContext.Close()
}
...
func TestFrameworkRunSubtaskCancel(t *testing.T) {
	...
	RegisterTaskMeta(t, ctrl, &m, &testDispatcherExt{})
	distContext := testkit.NewDistExecutionContext(t, 3)
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/MockRunSubtaskCancel", "1*return(true)"))
	DispatchTaskAndCheckState(ctx, "key1", t, &m, proto.TaskStateReverted)
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/MockRunSubtaskCancel"))
	distContext.Close()
}
...
func TestFrameworkCleanUpRoutine(t *testing.T) {
	...
	RegisterTaskMeta(t, ctrl, &m, &testDispatcherExt{})
	distContext := testkit.NewDistExecutionContext(t, 3)
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/dispatcher/WaitCleanUpFinished", "return()"))
	DispatchTaskAndCheckSuccess(ctx, "key1", t, &m)
...
func TestTaskCancelledBeforeUpdateTask(t *testing.T) {
	...
	RegisterTaskMeta(t, ctrl, &m, &testDispatcherExt{})
	distContext := testkit.NewDistExecutionContext(t, 1)
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/dispatcher/cancelBeforeUpdateTask", "1*return(true)"))
	DispatchTaskAndCheckState(ctx, "key1", t, &m, proto.TaskStateReverted)
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/dispatcher/cancelBeforeUpdateTask"))
	distContext.Close()
}
