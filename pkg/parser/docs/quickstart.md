...
First, you need to use `go get` to fetch the dependencies through git hash. The git hashes are available in [release page](https://github.com/ocean2811/tidbeaff0fbc576a/releases). Take `v5.3.0` as an example:

```bash
go get -v github.com/ocean2811/tidbeaff0fbc576a/parser@4a1b2e9
```
...
>
> ```bash
> go get -v github.com/ocean2811/tidbeaff0fbc576a/types/parser_driver@4a1b2e9
> ```
> and import it in your golang source code:
> ```go
> import _ "github.com/ocean2811/tidbeaff0fbc576a/types/parser_driver"
> ```
...
1. Use the [`parser.New()`](https://pkg.go.dev/github.com/ocean2811/tidbeaff0fbc576a/parser?tab=doc#New) function to instantiate a parser, and
2. Invoke the method [`Parse(sql, charset, collation)`](https://pkg.go.dev/github.com/ocean2811/tidbeaff0fbc576a/parser?tab=doc#Parser.Parse) on the parser.
...
import (
	...
	"github.com/ocean2811/tidbeaff0fbc576a/parser"
	"github.com/ocean2811/tidbeaff0fbc576a/parser/ast"
	_ "github.com/ocean2811/tidbeaff0fbc576a/parser/test_driver"
)
...
>
>   You can use [`github.com/ocean2811/tidbeaff0fbc576a/parser/test_driver`](https://pkg.go.dev/github.com/ocean2811/tidbeaff0fbc576a/parser/test_driver) as the `parser_driver` for test. Again, if you need advanced features, please use the `parser_driver` in TiDB (run `go get -v github.com/ocean2811/tidbeaff0fbc576a/types/parser_driver@4a1b2e9` and import it).
...
> - The 2nd and 3rd arguments of [`parser.Parse()`](https://pkg.go.dev/github.com/ocean2811/tidbeaff0fbc576a/parser?tab=doc#Parser.Parse) are charset and collation respectively. If you pass an empty string into it, a default value is chosen.
...
Parser implements the interface [`ast.Node`](https://pkg.go.dev/github.com/ocean2811/tidbeaff0fbc576a/parser/ast?tab=doc#Node) for each kind of AST node, such as SelectStmt, TableName, ColumnName. [`ast.Node`](https://pkg.go.dev/github.com/ocean2811/tidbeaff0fbc576a/parser/ast?tab=doc#Node) provides a method `Accept(v Visitor) (node Node, ok bool)` to allow any struct that has implemented [`ast.Visitor`](https://pkg.go.dev/github.com/ocean2811/tidbeaff0fbc576a/parser/ast?tab=doc#Visitor) to traverse itself.

[`ast.Visitor`](https://pkg.go.dev/github.com/ocean2811/tidbeaff0fbc576a/parser/ast?tab=doc#Visitor) is defined as follows:
```go
