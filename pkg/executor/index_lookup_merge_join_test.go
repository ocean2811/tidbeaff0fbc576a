...
func TestIndexLookupMergeJoinHang(t *testing.T) {
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/executor/IndexMergeJoinMockOOM", `return(true)`))
	defer func() {
		require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/executor/IndexMergeJoinMockOOM"))
	}()
...
func TestIssue18068(t *testing.T) {
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/executor/testIssue18068", `return(true)`))
	defer func() {
		require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/executor/testIssue18068"))
	}()
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
