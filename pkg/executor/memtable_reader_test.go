...
func TestMetricTableData(t *testing.T) {
	...
	fpName := "github.com/ocean2811/tidbeaff0fbc576a/pkg/executor/mockMetricsPromData"
...
func TestTiDBClusterConfig(t *testing.T) {
	...
	fpName := "github.com/ocean2811/tidbeaff0fbc576a/pkg/executor/mockClusterConfigServerInfo"
	...
	for _, r := range rs {
		...
		// https://github.com/ocean2811/tidbeaff0fbc576a/issues/34867
...
func TestTiDBClusterLog(t *testing.T) {
	...
	fpName := "github.com/ocean2811/tidbeaff0fbc576a/pkg/executor/mockClusterLogServerInfo"
	fpExpr := strings.Join(servers, ";")
	require.NoError(t, failpoint.Enable(fpName, fmt.Sprintf(`return("%s")`, fpExpr)))
	defer func() { require.NoError(t, failpoint.Disable(fpName)) }()
