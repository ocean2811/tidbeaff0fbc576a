...
func TestReorgPartitionRollback(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/ddl/mockUpdateVersionAndTableInfoErr", `return(true)`))
	...
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/ddl/reorgPartitionAfterDataCopy", `return(true)`))
	defer func() {
		err := failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/ddl/reorgPartitionAfterDataCopy")
		require.NoError(t, err)
	}()
	tk.MustExecToErr("alter table t reorganize partition p1 into (partition p1a values less than (15), partition p1b values less than (20))")
	tk.MustExec(`admin check table t`)
