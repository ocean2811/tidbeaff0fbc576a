...
func TestAbnormalSessionPool(t *testing.T) {
	...
	pool := domain.SysSessionPool()
	failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/domain/mockSessionPoolReturnError", "return")
	se, err := pool.Get()
	require.Error(t, err)
	failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/domain/mockSessionPoolReturnError")
	require.Equal(t, svr.InternalSessionExists(se), false)
}
