...
func TestSetAndGetOwnerOpValue(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/owner/MockDelOwnerKey", `return("delOwnerKeyAndNotOwner")`))
	...
	require.Equal(t, op, owner.OpNone)
	require.False(t, op.IsSyncedUpgradingState())
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/owner/MockDelOwnerKey"))
	...
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/owner/MockDelOwnerKey", `return("onlyDelOwnerKey")`))
	err = manager.SetOwnerOpValue(context.Background(), owner.OpSyncUpgradingState)
	require.Error(t, err, "put owner key failed, cmp is false")
	isOwner = checkOwner(tInfo.ddl, true)
	require.True(t, isOwner)
...
func TestGetOwnerOpValueBeforeSet(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/owner/MockNotSetOwnerOp", `return(true)`))
	...
	require.NoError(t, err)
	require.Equal(t, op, owner.OpNone)
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/owner/MockNotSetOwnerOp"))
	err = manager.SetOwnerOpValue(context.Background(), owner.OpSyncUpgradingState)
	require.NoError(t, err)
	op, err = owner.GetOwnerOpValue(context.Background(), nil, DDLOwnerKey, "log prefix")
	require.NoError(t, err)
