...
func TestSyncLoadSkipUnAnalyzedItems(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/statistics/handle/assertSyncLoadItems", `return(0)`))
	tk.MustQuery("trace plan select * from t where a > 10")
	failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/statistics/handle/assertSyncLoadItems")
	tk.MustExec("analyze table t1")
	// one column would be loaded
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/statistics/handle/assertSyncLoadItems", `return(1)`))
...
func TestConcurrentLoadHistWithPanicAndFail(t *testing.T) {
	...
	failpoints := []struct {
		...
		{
			failPath: "github.com/ocean2811/tidbeaff0fbc576a/pkg/statistics/handle/mockReadStatsForOnePanic",
			inTerms:  "panic",
		},
		{
			failPath: "github.com/ocean2811/tidbeaff0fbc576a/pkg/statistics/handle/mockReadStatsForOneFail",
			inTerms:  "return(true)",
		},
	}
...
func TestRetry(t *testing.T) {
	...
	exitCh := make(chan struct{})
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/pkg/statistics/handle/mockReadStatsForOneFail", "return(true)"))
	var (
	...
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/pkg/statistics/handle/mockReadStatsForOneFail"))
}
