...
func TestResolveLock(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/br/pkg/streamhelper/NeedResolveLocks", `return(true)`))
	// make sure asyncResolveLocks stuck in optionalTick later.
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/br/pkg/streamhelper/AsyncResolveLocks", `pause`))
	defer func() {
		require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/br/pkg/streamhelper/NeedResolveLocks"))
	}()
	...
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/br/pkg/streamhelper/AsyncResolveLocks"))
...
func TestOwnerDropped(t *testing.T) {
	...
	fp := "github.com/ocean2811/tidbeaff0fbc576a/br/pkg/streamhelper/get_subscriber"
	defer func() {
		if t.Failed() {
			fmt.Println(c)
		}
...
func TestRemoveTaskAndFlush(t *testing.T) {
	...
	adv.SpawnSubscriptionHandler(ctx)
	require.NoError(t, adv.OnTick(ctx))
	require.NoError(t, failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/br/pkg/streamhelper/subscription-handler-loop", "pause"))
	c.flushAll()
	env.unregisterTask()
	require.Eventually(t, func() bool {
		return !adv.HasTask()
	}, 10*time.Second, 100*time.Millisecond)
	require.NoError(t, failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/br/pkg/streamhelper/subscription-handler-loop"))
	require.Eventually(t, func() bool {
...
func TestOwnershipLost(t *testing.T) {
	...
	c.advanceCheckpoints()
	c.flushAll()
	failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/br/pkg/streamhelper/subscription.listenOver.aboutToSend", "pause")
	failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/br/pkg/streamhelper/FlushSubscriber.Clear.timeoutMs", "return(500)")
	wg := new(sync.WaitGroup)
	wg.Add(adv.TEST_registerCallbackForSubscriptions(wg.Done))
	cancel()
	failpoint.Disable("github.com/ocean2811/tidbeaff0fbc576a/br/pkg/streamhelper/subscription.listenOver.aboutToSend")
	wg.Wait()
}
...
func TestSubscriptionPanic(t *testing.T) {
	...
	require.NoError(t, adv.OnTick(ctx))
	failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/br/pkg/streamhelper/subscription.listenOver.aboutToSend", "5*panic")
