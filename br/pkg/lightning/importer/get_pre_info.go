...
import (
	...
	_ "github.com/ocean2811/tidbeaff0fbc576a/pkg/planner/core" // to setup expression.EvalAstExpr. Otherwise we cannot parse the default value
...
func (p *PreImportInfoGetterImpl) getTableStructuresByFileMeta(ctx context.Context, dbSrcFileMeta *mydump.MDDatabaseMeta, getPreInfoCfg *ropts.GetPreInfoConfig) ([]*model.TableInfo, error) {
	...
	failpoint.Inject(
		...
		func(v failpoint.Value) {
			...
			//nolint: errcheck
			failpoint.Enable("github.com/ocean2811/tidbeaff0fbc576a/br/pkg/lightning/backend/tidb/FetchRemoteTableModels_BeforeFetchTableAutoIDInfos", fmt.Sprintf("sleep(%d)", sleepMilliSeconds))
		},
	)
