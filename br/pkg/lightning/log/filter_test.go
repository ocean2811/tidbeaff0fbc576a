...
func TestFilter(t *testing.T) {
	...
	logger, buffer = log.MakeTestLogger(zap.WrapCore(func(c zapcore.Core) zapcore.Core {
		return log.NewFilterCore(c, "github.com/ocean2811/tidbeaff0fbc576a/br/").With([]zap.Field{zap.String("a", "b")})
	}), zap.AddCaller())
	logger.Warn("the message", zap.Int("number", 123456), zap.Ints("array", []int{7, 8, 9}))
	...
	logger.Warn("the message", zap.String("stack", "github.com/ocean2811/tidbeaff0fbc576a/br/"))
	require.Len(t, buffer.Stripped(), 0)
}
...
//
// Run `go test -run='^$' -bench=. -v github.com/ocean2811/tidbeaff0fbc576a/br/pkg/lightning/log` to get benchmark result.
func BenchmarkFilterStringsContains(b *testing.B) {
	b.ResetTimer()

	inputs := []string{
		"github.com/ocean2811/tidbeaff0fbc576a/some/package/path",
		"github.com/tikv/pd/some/package/path",
		"github.com/ocean2811/tidbeaff0fbc576a/br/some/package/path",
	}
	filters := []string{"github.com/ocean2811/tidbeaff0fbc576a/", "github.com/tikv/pd/"}
	for i := 0; i < b.N; i++ {
		for i := range inputs {
			for j := range filters {
