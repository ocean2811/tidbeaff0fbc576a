...
for failure in $cases; do
    ...
    export GO_FAILPOINTS="github.com/ocean2811/tidbeaff0fbc576a/br/pkg/backup/hint-backup-start=1*return(\"$hint_backup_start\");\
github.com/ocean2811/tidbeaff0fbc576a/br/pkg/backup/hint-fine-grained-backup=1*return(\"$hint_finegrained\");\
github.com/ocean2811/tidbeaff0fbc576a/br/pkg/utils/hint-get-backup-client=1*return(\"$hint_get_backup_client\")"
    if [ "$failure" = outage-at-finegrained ]; then
        export GO_FAILPOINTS="$GO_FAILPOINTS;github.com/ocean2811/tidbeaff0fbc576a/br/pkg/backup/noop-backup=return(true)"
    fi
