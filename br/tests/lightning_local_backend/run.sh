...
export GO_FAILPOINTS='github.com/ocean2811/tidbeaff0fbc576a/br/pkg/lightning/backend/local/FailIngestMeta=1*return("notleader");github.com/ocean2811/tidbeaff0fbc576a/br/pkg/lightning/backend/local/failToSplit=5*return("")'
...
export GO_FAILPOINTS='github.com/ocean2811/tidbeaff0fbc576a/br/pkg/lightning/backend/local/FailIngestMeta=2*return("epochnotmatch");github.com/ocean2811/tidbeaff0fbc576a/br/pkg/lightning/backend/local/NoLeader=1*return()'
...
set +e
export GO_FAILPOINTS='github.com/ocean2811/tidbeaff0fbc576a/br/pkg/lightning/importer/FailIfStatusBecomes=return(90);'
...
for ckpt in mysql file; do
  ...
  set +e
  export GO_FAILPOINTS="github.com/ocean2811/tidbeaff0fbc576a/br/pkg/lightning/importer/FailAfterWriteRows=return"
  ...
  rm -f "/tmp/tidb_lightning_checkpoint_local_backend_test.pb"
  set +e
  export GO_FAILPOINTS="github.com/ocean2811/tidbeaff0fbc576a/br/pkg/lightning/importer/LocalBackendSaveCheckpoint=return;github.com/ocean2811/tidbeaff0fbc576a/br/pkg/lightning/importer/FailIfImportedChunk=return"
  run_lightning --backend local --enable-checkpoint=1 --log-file "$TEST_DIR/lightning-local.log" --config "$CUR/$ckpt.toml"
  set -e
  run_lightning_ctl --check-local-storage \
    --backend local \
  ...
  set +e
  export GO_FAILPOINTS="github.com/ocean2811/tidbeaff0fbc576a/br/pkg/lightning/importer/FailIfIndexEngineImported=return(1)"
  run_lightning --backend local --enable-checkpoint=1 --log-file "$TEST_DIR/lightning-local.log" --config "$CUR/$ckpt.toml"
  set -e
  run_lightning_ctl --check-local-storage \
    --backend local \
