...
# See https://github.com/ocean2811/tidbeaff0fbc576a/pull/30692.
...
ifeq ("$(pkg)", "")
	...
	@echo "Running unit test for github.com/ocean2811/tidbeaff0fbc576a/$(pkg)"
	@export log_level=fatal; export TZ='Asia/Shanghai'; \
	$(GOTEST) -tags 'intest' -v -ldflags '$(TEST_LDFLAGS)' -cover github.com/ocean2811/tidbeaff0fbc576a/$(pkg) || { $(FAILPOINT_DISABLE); exit 1; }
endif
	@$(FAILPOINT_DISABLE)
...
bench-daily:
	go test -tags intest github.com/ocean2811/tidbeaff0fbc576a/pkg/session -run TestBenchDaily -bench Ignore --outfile bench_daily.json
	go test -tags intest github.com/ocean2811/tidbeaff0fbc576a/pkg/executor -run TestBenchDaily -bench Ignore --outfile bench_daily.json
	go test -tags intest github.com/ocean2811/tidbeaff0fbc576a/pkg/executor/test/splittest -run TestBenchDaily -bench Ignore --outfile bench_daily.json
	go test -tags intest github.com/ocean2811/tidbeaff0fbc576a/pkg/tablecodec -run TestBenchDaily -bench Ignore --outfile bench_daily.json
	go test -tags intest github.com/ocean2811/tidbeaff0fbc576a/pkg/expression -run TestBenchDaily -bench Ignore --outfile bench_daily.json
	go test -tags intest github.com/ocean2811/tidbeaff0fbc576a/pkg/util/rowcodec -run TestBenchDaily -bench Ignore --outfile bench_daily.json
	go test -tags intest github.com/ocean2811/tidbeaff0fbc576a/pkg/util/codec -run TestBenchDaily -bench Ignore --outfile bench_daily.json
	go test -tags intest github.com/ocean2811/tidbeaff0fbc576a/pkg/distsql -run TestBenchDaily -bench Ignore --outfile bench_daily.json
	go test -tags intest github.com/ocean2811/tidbeaff0fbc576a/pkg/util/benchdaily -run TestBenchDaily -bench Ignore \
...
build_for_br_integration_test:
	...
	($(GOTEST) -c -cover -covermode=count \
		-coverpkg=github.com/ocean2811/tidbeaff0fbc576a/br/... \
		-o $(BR_BIN).test \
		github.com/ocean2811/tidbeaff0fbc576a/br/cmd/br && \
	$(GOTEST) -c -cover -covermode=count \
		-coverpkg=github.com/ocean2811/tidbeaff0fbc576a/br/... \
		-o $(LIGHTNING_BIN).test \
		github.com/ocean2811/tidbeaff0fbc576a/br/cmd/tidb-lightning && \
	$(GOTEST) -c -cover -covermode=count \
		-coverpkg=github.com/ocean2811/tidbeaff0fbc576a/br/... \
		-o $(LIGHTNING_CTL_BIN).test \
		github.com/ocean2811/tidbeaff0fbc576a/br/cmd/tidb-lightning-ctl && \
...
mock_lightning: tools/bin/mockgen
	tools/bin/mockgen -package mock github.com/ocean2811/tidbeaff0fbc576a/br/pkg/lightning/backend Backend,EngineWriter,TargetInfoGetter,ChunkFlushStatus > br/pkg/mock/backend.go
	tools/bin/mockgen -package mock github.com/ocean2811/tidbeaff0fbc576a/br/pkg/lightning/backend/encode Encoder,EncodingBuilder,Rows,Row > br/pkg/mock/encode.go
	tools/bin/mockgen -package mocklocal github.com/ocean2811/tidbeaff0fbc576a/br/pkg/lightning/backend/local DiskUsage,TiKVModeSwitcher > br/pkg/mock/mocklocal/local.go
	tools/bin/mockgen -package mock github.com/ocean2811/tidbeaff0fbc576a/br/pkg/utils TaskRegister > br/pkg/mock/task_register.go

gen_mock: tools/bin/mockgen
	tools/bin/mockgen -package mock github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler TaskTable,Pool,Scheduler,Extension > pkg/disttask/framework/mock/scheduler_mock.go
	tools/bin/mockgen -package mock github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/dispatcher Dispatcher,CleanUpRoutine,TaskManager > pkg/disttask/framework/mock/dispatcher_mock.go
	tools/bin/mockgen -package execute github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/scheduler/execute SubtaskExecutor > pkg/disttask/framework/mock/execute/execute_mock.go
	tools/bin/mockgen -package mock github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/importinto MiniTaskExecutor > pkg/disttask/importinto/mock/import_mock.go
	tools/bin/mockgen -package mock github.com/ocean2811/tidbeaff0fbc576a/pkg/disttask/framework/planner LogicalPlan,PipelineSpec > pkg/disttask/framework/mock/plan_mock.go
	tools/bin/mockgen -package mock github.com/ocean2811/tidbeaff0fbc576a/pkg/util/sqlexec RestrictedSQLExecutor > pkg/util/sqlexec/mock/restricted_sql_executor_mock.go
...
data_parsers: tools/bin/vfsgendev br/pkg/lightning/mydump/parser_generated.go br_web
	...
	tools/bin/vfsgendev -source='"github.com/ocean2811/tidbeaff0fbc576a/br/pkg/lightning/web".Res' && mv res_vfsdata.go br/pkg/lightning/web/
